...
class OrderBoardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $fetchOwn = function (int $sFrom, int $sTo) {
            return OwnOrder::with(['ownOrderProducts.product','user','costumer'])
                ->whereExists(function ($q) use ($sFrom, $sTo) {
                    $q->select(DB::raw(1))
                      ->from('own_order_states')
                      ->whereColumn('own_order_states.own_order_id', 'own_orders.id')
                      ->whereBetween('own_order_states.state_id', [$sFrom, $sTo])
                      ->where('own_order_states.selected', 'yes');
                });
        };

        // helper to get maquila orders matching a states range
        $fetchMaquila = function (int $sFrom, int $sTo) {
            return MaquilaOrder::with(['maquila_meshes.mesh','maquila_services.service','user','costumer'])
                ->whereExists(function ($q) use ($sFrom, $sTo) {
                    $q->select(DB::raw(1))
                      ->from('maquila_order_states')
                      ->whereColumn('maquila_order_states.maquila_order_id', 'maquila_orders.id')
                      ->whereBetween('maquila_order_states.state_id', [$sFrom, $sTo])
                      ->where('maquila_order_states.selected', 'yes');
                });
        };

        // Area definitions
        $area1Own = $fetchOwn(1,4)->get();
        $area1Maquila = $fetchMaquila(1,4)->get();
        ...
        $area2Own = $fetchOwn(4,5)->get();
        $area2Maquila = $fetchMaquila(4,5)->get();

        $area3Own = $fetchOwn(6,10)->get();
        $area3Maquila = $fetchMaquila(6,10)->get();
